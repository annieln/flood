...
class Model {
     ...
     *
     * Cases of PLACE:
     *  1. Place is in the top-left corner: pl(0,0)
     *  --> In this case, place only has down, right
     *  2. Place is in the top-right corner: pl(0, _width - 1)
     *  --> In this case, Place only has down, left
     *  3. Place is in the bottom-left corner: pl(_height - 1, 0)
     *  --> In this case, Place only has up, right
     *  4. Place is in the bottom-right corner: pl(_height - 1, width - 1)
     *  --> In this case, Place only has up, left
     *  5. Place is in the first row: pl(0, col)
     *  --> All options but up
     *  6. Place is in the first column: pl(row, 0)
     *  --> All options but left
     *  7. Place is in the last row: pl(_height - 1, col)
     *  --> All options but down
     *  8. Place is in the last column: pl(row, _width - 1)
     *  --> All options but right
     *  9. If none of the previous conditions, apply on all directions.
     * */
    void forNeighbors(Place place, Consumer<Place> action) {
        ...
        } else if (place.row == 0 && place.col == _width - 1) {
            action.accept(pl(place.row + 1, place.col));
            action.accept(pl(place.row, place.col - 1));
        } else if (place.row == _height - 1 && place.col == 0) {
            action.accept(pl(place.row - 1, place.col));
            action.accept(pl(place.row, place.col + 1));
     ...
     *
     *  Method:
     *  Create a boolean array of size _ncolors. Then, check if each
     *  color of _ncolors matches the color of some cell on the grid, or
     *  some _cells[r][c].
     *
     *  Note: r spans _height and c spans _width.
     *
     **/
    boolean[] colorsPresent() {
        boolean[] colorChecker = new boolean[_ncolors];
        for (int i = 0; i < _ncolors; i++) {
            for (int r = 0; r < _height; r++) {
                for (int c = 0; c < _width; c++) {
                    if (i == _cells[r][c]) {
                        colorChecker[i] = Boolean.TRUE;
                        continue;
                    }
                }
            }
    ...
    final boolean solved() {
        return (_active.size() == (_width * _height));
    }
    ...
    Set<Place> adjacentCells(int color) {
        ...
        HashSet<Place> adjacent = new HashSet<Place>(_active);
        undo();
        adjacent.removeAll(_active);
        return adjacent;
    }
